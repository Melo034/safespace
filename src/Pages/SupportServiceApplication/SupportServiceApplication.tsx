...
const SupportServiceApplication = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.type || !VALID_TYPES.includes(formData.type as ServiceType))
      return "Valid service type is required.";
    if (!formData.title.trim()) return "Title is required.";
    if (!formData.specialization.trim()) return "Specialization is required.";
    if (!formData.description.trim()) return "Description is required.";
    if (!formData.address.trim()) return "Address is required.";
    if (!formData.phone.trim() || !phoneRegex.test(formData.phone))
      return "Valid phone number is required (e.g., +232456789).";
    if (!formData.email.trim() || !emailRegex.test(formData.email)) return "Valid email is required.";
    if (!formData.availability || !VALID_AVAILABILITY.includes(formData.availability as AvailabilityType))
    ...
    if (formData.website.trim()) {
      try {
        ...
        new URL(formData.website.trim() ? normalizeWebsite(formData.website) : "");
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        availability: formData.availability as AvailabilityType,
        languages: toCSVArray(formData.languages),
        credentials: formData.credentials.trim(),
        tags: toCSVArray(formData.tags),
        status: "pending",
